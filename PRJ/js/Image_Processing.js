...
class Picture {
  ...
  build_Color_Rect(cnv, hist, redColor, greenColor, blueColor) {
    ...
    for (let c = 0; c < redColor.length; c++) {
      ctx.fillStyle =
        "rgb(" + redColor[c] + "," + greenColor[c] + "," + blueColor[c] + ")";
...
class ColorHistogram {
  constructor(redColor, greenColor, blueColor) {
    ...
    this.limiar1 = 160;
    this.limiar2 = 70;
    this.hist = Array(12).fill(0);
  }

  /**
   * Para cada pixel faz a diferença de cada cor, e se está dentro dos parâmetros, incrementa no array histo
   * na posição da cor.
   * @param {pixeis da imagem} pixels
   * @returns
   */
  count_Pixels(pixels) {
